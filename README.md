...
在 `.md` 文件顶部填写以下初始化信息，即可完成标题（```title```），标签（```tags```），分类（```categories```）的设置，**其中```title```为必填项目**（这些关键词不是我定义的，我借用了著名静态博客构建工具 [hexo](https://github.com/hexojs/hexo) 的标准）
